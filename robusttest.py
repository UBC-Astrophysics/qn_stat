...
if True:
    ...
    print('robust.robustcurve_fit with dorobust=False (standard chi2):')
    print(robustcurve_fit(funk,xdata,ydata,dorobust=False))
    print('robust.robustcurve_fit with dorobust=True (default is Qn and HL):')
    print(robustcurve_fit(funk,xdata,ydata))
    print('scipy.optimize.curve_fit with starting value and sigma:')
    print(curve_fit(funk,xdata,ydata,p0=[4.0,4.0,4.0],sigma=0*xdata+1))
    print('robust.robustcurve_fit with starting value, sigma, dorobust=True, different method and return res:')
